...
class AudioFileDataset(Dataset):
    def __init__(self,
                 train_f_paths = '../LibriSpeech_preprocess/dataset/train.tsv',
                 valid_f_paths = '../LibriSpeech_preprocess/dataset/valid.tsv',
                 train_ltr_path = '../LibriSpeech_preprocess/dataset/train.ltr',
                 valid_ltr_path = '../LibriSpeech_preprocess/dataset/valid.ltr',
        ...
        self.device = device
        self.root = '/remote/idiap.svm/resource.dataset02/LibriSpeech/train-clean-100/'
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            wav_input, _ = soundfile.read(os.path.join(self.root, filename[0]))
            ...
            if wav_len>self.max_len:
                padding_mask = torch.zeros(self.max_len, dtype=torch.float32)
                wav_input = torch.torch.tensor(wav_input[:self.max_len], dtype=torch.float32)
            else:
                padding_mask = F.pad(torch.zeros(len(wav_input), dtype=torch.float32),
                                     (0,self.max_len-wav_len), mode='constant', value=1)
                wav_input = F.pad(torch.torch.tensor(wav_input, dtype=torch.float32),
            ...
            sample['net_input'] = wav_input
            sample['padding_mask'] = padding_mask
            sample['target'] = torch.tensor(tokens)
